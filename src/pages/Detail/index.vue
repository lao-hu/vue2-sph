        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="list in spuSaleAttrList" :key="list.id" >
                <dt class="title">{{ list.saleAttrName }}</dt>
                <!-- <dd changepirce="0" class="active">金色</dd> -->
                <dd
                  changepirce="40"
                  v-for="(res) in list.spuSaleAttrValueList"
                  :key="res.id"
                  :class="{ active: res.isChecked == 1 }"
                   @click="changeChecked(res,list.spuSaleAttrValueList)"
                >
                  {{ res.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
export default {
  ...
  methods:{
    ...
    */
  async addShopCar(){
      // 1. 发送请求：将产品存入数据库（通知服务器）
     // 调用仓库里面的 async addOrUpdateShopCart 函数，返回一个promise对象
     // 要么成功，要么失败
      ...
      try {
        ...
        //  2. 服务存储成功：--- 进行路由跳转
       /* 
        路由跳转时候，可以携带参数，简单的参数，通过query形式带给组件
        复杂的信息数据，可以通过会话存储
      */
       // 会话存储数据
  ...
  ,mounted() {
    ...
    // 判断路由中的params参数是否存在，如果存在就发起请求
    if ("id" in this.$route.params) {
      this.$store.dispatch("detail/goodsInfo", this.$route.params.id);
    }
